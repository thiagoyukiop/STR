...
volatile clock_t TEMPO_INTERFERENCIA_START_ABS = 0;
volatile clock_t TEMPO_INTERFERENCIA_START_AIRBAG = 0;

volatile clock_t TEMPO_INTERFERENCIA_ABS = 0;
volatile clock_t TEMPO_INTERFERENCIA_AIRBAG = 0;
        ...
        if(kbhit()) {
            ...
            if(tecla == 'b') {
                ...
                if(SENSOR_AIRBAG == false) {
                    TEMPO_INTERFERENCIA_START_AIRBAG = clock();
                    SENSOR_AIRBAG = true;
                ...
                pthread_mutex_lock(&mutex);
                if(SENSOR_ABS == false) {
                    TEMPO_INTERFERENCIA_START_ABS = clock();
                    SENSOR_ABS = true;
...
void *thread_abs_func(void *data) {
    ...
    uint64_t execution_time, total_time, total_execution_time = 0, total_interference_time = 0;
    const int m = 8, k = 10;
    ...
    while (1) {
        ...
        if(SENSOR_ABS) {
            ...
            total_interference_time = (uint64_t)(((double)(start_time - TEMPO_INTERFERENCIA_START_ABS) / CLOCKS_PER_SEC) * 1e6);

            TEMPO_INTERFERENCIA_ABS = total_interference_time / count;
...
void *thread_airbag_func(void *data) {
    ...
    while (1) {
        ...
        if(SENSOR_AIRBAG) {
            ...
            total_interference_time += (uint64_t)(((double)(start_time - TEMPO_INTERFERENCIA_START_AIRBAG) / CLOCKS_PER_SEC) * 1e6);
            
            TEMPO_INTERFERENCIA_AIRBAG = total_interference_time / count;
...
void *thread_display_func(void *data) {
    ...
    while (1) {
        ...
        printf("\033[34m%d:Tempo de Interferência do ABS: %" PRIu64 " us\033[0m\n", count, TEMPO_INTERFERENCIA_ABS);
        ...
        printf("\033[35m%d:Tempo de Interferência do Airbag: %" PRIu64 " us\033[0m\n", count, TEMPO_INTERFERENCIA_AIRBAG);
